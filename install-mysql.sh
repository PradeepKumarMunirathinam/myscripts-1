...
sudo mysql-server-5.6.13 mysql-server/root_password password welcome@123 | debconf-set-selections
sudo mysql-server-5.6.13 mysql-server/root_password_again password welcome@123 | debconf-set-selections
...
sudo "GRANT ALL PRIVILEGES ON *.* TO 'root'@'%' IDENTIFIED BY 'welcome@123'; flush privileges;" | mysql -u root -pwelcome@123
